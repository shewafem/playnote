            ...
            for (const chord of chordsByKey) {
                ...
                if (existingChord) {
                    ...
                    continue; // Пропускаем, если аккорд уже есть, чтобы избежать дубликатов
                              // Или можно реализовать логику обновления, если нужно
                }
                ...
                try {
                    const createdChord = await prisma.chord.create({
                        data: {
                            key: chord.key,
                            suffix: chord.suffix,
                            positions: { // Prisma позволяет создавать связанные записи таким образом
                        ...
                        include: { // Опционально: включить созданные позиции в результат
                            positions: true,
                        },
                    });
                    console.log(`Создан аккорд: ${createdChord.key} ${createdChord.suffix} с ${createdChord.positions.length} позициями.`);
                } catch (e) {
